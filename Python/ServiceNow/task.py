...
def insert_data_into_sql(data):
    try:
        ...
        for record in data:
            # Assuming 'tasks' table exists in the Azure Synapse SQL database with appropriate schema
            cursor.execute("INSERT INTO tasks (column1, column2, ...) VALUES (?, ?, ...)", 
                           (record['column1'], record['column2'], ...))  # Replace column names accordingly
