...
$Fields = @("All")
$DateTimeToAESTs = @("sys_created_on","sys_updated_on","last_login_time")
$Fields += $DateTimeToAESTs

$IndexExemption = "sys_id"  ##Default:sys_id should always be included for Primary key configuration
...
do {
    ...
    $endpoint = "/api/now/v1/table/$($entity)?sysparm_limit=$limit&sysparm_offset=$i&sysparm_query=sys_updated_onRELATIVEGE@hour@ago@2&sysparm_display_value=all" `
                + "$($fieldsquery)"
